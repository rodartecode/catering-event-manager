...
import { cleanTestDatabase, seedTestClient, seedTestUser } from '../helpers/db';
...
test.describe('Event Lifecycle', () => {
  ...
  test.describe
    .serial('Create and Progress Event', () => {
      ...
      test('update event status: inquiry → planning', async ({ page }) => {
        ...
        await expect(page.locator('h3:has-text("Update Event Status")')).not.toBeVisible({
          timeout: 10000,
        });
      ...
      test('archive completed event', async ({ page }) => {
        ...
        await page
          ...
          .last()
          .locator('button:has-text("Archive Event")')
          .click();
