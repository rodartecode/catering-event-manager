...
import { cleanTestDatabase, seedTestUser, seedTestClient, seedTestEvent, seedTestTask } from '../helpers/db';
...
test.describe('Task Management', () => {
  ...
  test.describe.serial('Task CRUD Operations', () => {
    let taskName: string;
  ...
  test.describe('Task Assignment', () => {
    test('assign task to user', async ({ page }) => {
      ...
      const userButton = page.locator('.fixed.inset-0').last()
        .locator('button:has-text("Test Admin")');
      await userButton.click();

      // Confirm assignment
      await page.locator('.fixed.inset-0').last()
        .locator('button:has-text("Assign")').last().click();
  ...
  test.describe('Overdue Tasks', () => {
    test('overdue task is visually marked', async ({ page }) => {
      ...
      const overdueTaskCard = page.locator('[data-testid="task-card"]:has-text("Overdue Test Task")');
