...
export async function createKitchenStation(
  ...
  overrides: Partial<{
    name: string;
    type: 'oven' | 'grill' | 'prep_counter' | 'cold_storage' | 'stovetop' | 'fryer' | 'mixer';
    capacity: number;
    venueId: number;
    notes: string;
    isActive: boolean;
  ...
  const id = nextId();
  const result = await db.execute(sql`
    INSERT INTO kitchen_stations (name, type, capacity, venue_id, notes, is_active)
    VALUES (
      ${overrides.name || `Test Station ${id}`},
      ${overrides.type || 'oven'},
    ...
    RETURNING id, name, type, capacity, venue_id, notes, is_active, created_at, updated_at
  `);
...
export async function createProductionTask(
  ...
  const id = nextId();
  const scheduledStartStr = overrides.scheduledStart
    ? overrides.scheduledStart.toISOString()
    : null;
  const scheduledEndStr = overrides.scheduledEnd ? overrides.scheduledEnd.toISOString() : null;
  const result = await db.execute(sql`
    INSERT INTO production_tasks (event_id, menu_item_id, station_id, name, prep_type, duration_minutes, offset_minutes, scheduled_start, scheduled_end, status, servings, assigned_to, notes, is_auto_generated)
    VALUES (
      ${eventId},
      ${overrides.menuItemId || null},
      ${overrides.stationId || null},
      ${overrides.name || `Test Prep Task ${id}`},
      ${overrides.prepType || 'chop'},
      ${overrides.durationMinutes || 30},
      ${overrides.offsetMinutes || -240},
      ${scheduledStartStr},
      ${scheduledEndStr},
      ${overrides.status || 'pending'},
      ${overrides.servings || null},
      ${overrides.assignedTo || null},
      ${overrides.notes || null},
      ${overrides.isAutoGenerated || false}
    )
    RETURNING id, event_id, menu_item_id, station_id, name, prep_type, duration_minutes, offset_minutes, scheduled_start, scheduled_end, status, servings, assigned_to, notes, is_auto_generated, created_at, updated_at
  `);
