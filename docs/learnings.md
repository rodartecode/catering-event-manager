...
### [2026-04-30] Demo DB schema drifted between Vercel deploys (no auto-migrate step)

**Problem**: First manual trigger of `/api/cron/reset-demo` after the public release returned `500: relation "production_tasks" does not exist`. The live demo DB was 4 migrations behind (missing venues/kitchen_stations/production_tasks/vendors/event_vendors) because Vercel deploys ship code but never run migrations against the demo DB. The `seedDemo()` cron assumed the schema was current and blew up at the first TRUNCATE referencing a missing table. The first scheduled Sunday firing (2026-05-03) would have failed identically.
**Solution**: Manually applied pending migrations via `psql -f` against the Supabase session pooler at `:5432` (drizzle-kit migrate hangs silently on both pooler ports — `:6543` is transaction-mode and doesn't support DDL transactions, `:5432` session-mode also fails inside drizzle-kit's wrapper). Backfilled `drizzle.__drizzle_migrations` with `sha256` of each migration's SQL content (drizzle-kit v0.31+ uses SHA256, not SHA1 as in the older entry above — the 12 already-applied rows showed both algorithms because the journal upgraded mid-history). Then PR #65 added a `runMigrations()` call to the cron route so future deploys self-heal.
**Context**: When schema files land on `main` but no one runs migrations against the demo DB, the next cron firing rots the demo silently. With PR #65, journal-tracked migrations apply automatically — but **hand-written SQL files not in `meta/_journal.json`** (`0001_events_trigger_and_view.sql`, `0003_resources.sql`, `0005_enable_rls.sql`, `0008_enable_missing_rls.sql`, `0012_enable_staff_rls.sql`, `0013_updated_at_triggers_and_fk_indexes.sql`) still need manual `psql -f`. Apply them BEFORE merging journal-tracked migrations that depend on objects they create — e.g., `0012_confused_puck.sql` references `update_updated_at_column()` defined in the hand-written `0013_updated_at_triggers...`. Verifying drizzle's hash format: `sha256sum migration.sql` should match the `hash` column in `drizzle.__drizzle_migrations` for any migration applied by drizzle-kit v0.31+.
