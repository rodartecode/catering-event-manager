...
The system SHALL have E2E tests that validate the complete authentication workflow including login, logout, and role-based access control. Tests for unimplemented UI features (error display on invalid/empty credentials) SHALL be skipped with documented reasons until the LoginForm surfaces auth errors.
...
#### Scenario: User login with invalid credentials (skipped)
...
- **NOTE** Skipped until LoginForm implements error display
...
The system SHALL have E2E tests that validate the complete event lifecycle from creation through archival. Tests SHALL use semantic HTML selectors and data-testid attributes matching the actual component implementation.
...
The system SHALL have E2E tests that validate task creation, assignment, and completion workflows. Tests for task status update buttons SHALL be skipped until TaskCard renders action buttons for status transitions.
...
- **THEN** the task is visually marked as overdue (identified by data-testid="overdue-indicator")

### Requirement: Resource Scheduling Workflow Testing

The system SHALL have E2E tests that validate resource CRUD and filtering. Tests for resource scheduling, conflict detection, and calendar view SHALL be skipped until the resource detail page and schedule calendar are implemented.
...
- **NOTE** Skipped until resource schedule calendar and conflict warning integration are built

#### Scenario: View resource schedule (skipped)
...
- **NOTE** Skipped until resource detail page with schedule view is implemented

### Requirement: Client Communication Workflow Testing

The system SHALL have E2E tests that validate client communication tracking. Tests for follow-up scheduling and completion SHALL be skipped until dedicated follow-up management UI is built.
...
#### Scenario: Schedule and complete follow-up (skipped)
...
- **NOTE** Skipped until dedicated follow-up management UI is built
